...
uses
  ...
  Vcl.Controls, Vcl.Forms, Vcl.Dialogs, Vcl.StdCtrls, ACBrAPIClient, ACBrAPIDTOs,
...
begin
  if cbAPI.ItemIndex = 1 then
    Result := 'https://prod.acbr.api.br/'
  else
    Result := 'https://hom.acbr.api.br/';
end;
...
begin
  ...
  Pdf := Client.Nfce.BaixarPdfNfce(NfceSelecionada, False, False,
    Format('Impresso em %s||ACBr API', [DateTimeToStr(Now)]),
    False, False, 80, '2');
...
begin
  ...
  edClientId.Text := GetEnvironmentVariable('ACBRAPI_CLIENTID');
  edClientSecret.Text := GetEnvironmentVariable('ACBRAPI_CLIENTSECRET');
